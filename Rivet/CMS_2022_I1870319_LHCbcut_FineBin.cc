...
namespace Rivet {
  ...
  class CMS_2022_I1870319_LHCbcut_FineBin: public Analysis {
  public:
    /// Constructor
    RIVET_DEFAULT_ANALYSIS_CTOR(CMS_2022_I1870319_LHCbcut_FineBin);
    ...
    void init() {
      ...
      //}
      book(_h_frag[0], "frag_fixed_bins", 100, 0.0, 1.0);
      book(_h_frag[1], "frag_fixed_bins_scaled", 100, 0.0, 1.0); // You should give it a different name
    }
    /// Perform the per-event analysis
    void analyze(const Event& event) {
      // first see if we have any prompt J/psi in the region
      Particles Jpsi;
      for (const Particle& p :apply<UnstableParticles>(event, "UFS").particles(Cuts::pid==443 && Cuts::etaIn(2.,4.5))) {
  ...
  RIVET_DECLARE_PLUGIN(CMS_2022_I1870319_LHCbcut_FineBin);
